...
var newline_placeholder = " @~@ ",
    newline_placeholder_t = newline_placeholder.trim();
...
sentences = function (text, user_options) {
    ...
    if (typeof user_options === "boolean") { options.newline_boundaries = true; }
    else {
    ...
    if (options.sanitize || options.allowed_tags) {
        if (!options.allowed_tags) { options.allowed_tags = [""]; }
    ...
    for (var i = 0, L = words.length; i < L; i++) {
        ...
        if (isBoundaryChar(words[i]) || endsWithChar(words[i], "?!") || words[i] == newline_placeholder_t) {
            if ((options.newline_boundaries || options.html_boundaries) && words[i] === newline_placeholder_t) { current.pop(); }
...
function sentencesIntersection(sent1, sent2, callback) {
    ...
    if ((s1.length + s2.length) === 0) { callback(true) }
...
function getSentencesRanks(content, callback, sentences_dict) {
    ...
    splitContentToSentences(content, function (sentences) {
        ...
        some(zeroNRange, function (i) {
            ...
            formatSentence(sentences[i], function (strip_s) { sentences_dict[strip_s] = score })
        })
...
setAbbreviations = function (abbr) {
    if (abbr) {
        abbreviations = abbr;
    } else { abbreviations = abbreviationArray; }
}

isSentenceStarter = function (str) {
    return (/^[A-Z][a-z].*/.test(str) || this.isNumber(str)) || /``|"|'/.test(str.substring(0, 2));
}
...
isNumber = function (str, dotPos) {
    if (dotPos) { str = str.slice(dotPos - 1, dotPos + 2); }
    return !isNaN(str);
};
...
isConcatenated = function (word) {
    ...
    if ((i = word.indexOf(".")) > -1 || (i = word.indexOf("!")) > -1 || (i = word.indexOf("?")) > -1) {
        ...
        if (c.match(/[a-zA-Z].*/)) { return [word.slice(0, i), word.slice(i + 1)]; }
    }
    return false;
};
...
endsWithChar = function ends_with_char(word, c) {
    if (c.length > 1) { return c.indexOf(word.slice(-1)) > -1; }
...
function createRange(fromRight) {
    return function (start, end, step) {
        if (step && typeof step != 'number') { end = step = undefined; }
...
function getText(filename) {
    ...
    f = fso.OpenTextFile(fso.GetParentFolderName(WScript.ScriptFullName) + filename, ForReading);
    return (f.ReadAll());
}
...
function createTextFile(text, filename) {
    var fso, f;
    fso = new ActiveXObject("Scripting.FileSystemObject");
    f = fso.CreateTextFile(fso.GetParentFolderName(WScript.ScriptFullName) + filename, true);
    f.WriteLine(text);
    f.Close();
}
...
summarize(title, content, 2, function (err, summary) {
    ...
    createTextFile(summary, "\\summary.txt");
})